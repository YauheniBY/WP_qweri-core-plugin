...
final class Elementor_Test_Extension {
	...
	public function admin_notice_missing_main_plugin() {
		...
		$message = sprintf(
			...
			esc_html__( '"%1$s" requires "%2$s" to be installed and activated.qwery' ),
			'<strong>' . esc_html__( 'Elementor Test Extensionqwery' ) . '</strong>',
			'<strong>' . esc_html__( 'Elementorqwery' ) . '</strong>'
		);
	...
	public function admin_notice_minimum_elementor_version() {
		...
		$message = sprintf(
			...
			esc_html__( '"%1$s" requires "%2$s" version %3$s or greater.qwery' ),
			'<strong>' . esc_html__( 'Elementor Test Extensionqwery' ) . '</strong>',
			'<strong>' . esc_html__( 'Elementorqwery' ) . '</strong>',
	...
	public function admin_notice_minimum_php_version() {
		...
		$message = sprintf(
			...
			esc_html__( '"%1$s" requires "%2$s" version %3$s or greater.qwery' ),
			'<strong>' . esc_html__( 'Elementor Test Extensionqwery' ) . '</strong>',
			'<strong>' . esc_html__( 'PHPqwery' ) . '</strong>',
